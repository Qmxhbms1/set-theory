...
\begin{solution}
  \begin{enumerate}[label=(\roman*)]
    \item Assume the opposite, let $A \cup B$ be D-infinite.
      This means that $A \cup B$ have a countable subset $S$, a sequence of elements $\{x_n | n < \omega\}$.
      Consider the sets $\{x_n \in A | n < \omega\}$ and $\{x_n \in B | n < \omega\}$.
      Clearly the union of these two sets is $S$, which is countable.
      If both of these sets were finite, it would follow that $S$ is also finite, hence at least one of them must be infinite, specifically countable (otherwise $S$ would be uncountable).
      Hence, either $A$ or $B$ have a countable subset which is equivalent to being D-infinite, a contradiction.
      Similarly for $A \times B$ we consider the projection of our finite subset onto $A$.
      If this set is infinite, we are done, otherwise there is some $\alpha$ such that $\alpha_n = \alpha$ for countably many $n$.
      This then implies that there must be countably many distinct $\beta_n$, otherwise we would have $(\alpha_n, \beta_n) = (\alpha_m, \beta_m)$ when $\alpha_m = \alpha_n = \alpha$.
      Thus it follows that $B$ has a countable subset and is thus D-infinite.
    \item Again, let us assume the opposite, that there is a countable subset of the set of all finite one-to-one sequences in a D-finite set.
      Consider the union of all the elements that appear in these sequences.
      This set must be infinite, otherwise we could only have finitely many sequences.
      % Unfinished
  \end{enumerate}
\end{solution}
